...
public class Menu {
		...
		while (opcao != 5) {
			...
			case 2:
				...
				while(op != 1 && op != 2) {
					...
					System.out.println("1 - Retroativo\t2 - Agora");
					op = read.nextInt(); 
	...
	// Médodo que cadastra um serviço/objeto no vetor (Agora)
		private static boolean cadastrarServicoAgora() throws ParseException {
			System.out.println("Para registrar um serviço, primeiro o pet deve estar cadastrado");
			System.out.println("Digite qual o id do pet/animal?");
			Pet animal = buscarAnimal(read.nextInt());
			if (animal != null) {
				System.out.println(animal.paraString());
				System.out.println("Funcionário\tDescrição");
				servicos[contServicos] = new Servico(animal, read.next(), read.next());
				return true;
			} else {
				System.out.println("Pet não encontrado.");
				return false;
